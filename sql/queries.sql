-- 1: Average salary of all lecturers
SELECT ROUND(AVG(salary), 2) AS average_salary
FROM lecturer
WHERE college_id = 1;

-- 2: Average salary and lecturer count per department
...
-- 3: Count of lecturers per degree type
SELECT degree,
       COUNT(*) AS count
FROM lecturer
WHERE college_id = 1
...
-- 4: All lecturers with their department (including unassigned)
...
-- 5: Number of articles per researcher (DR and PROF only)
SELECT l.name,
       l.degree,
...
-- 6: Committees with chairman and number of members
...
-- 7: Total articles written by members of each committee
SELECT c.name                         AS committee,
...
-- 8: Lecturers not assigned to any department
SELECT l.name, l.degree, l.salary
FROM lecturer l
WHERE l.dept_id IS NULL
  AND l.college_id = 1;

-- 9: Lecturers who belong to more than one committee
SELECT l.name,
       l.degree,
...
-- 11: Full committee membership report
SELECT c.name   AS committee,
       ch.name  AS chairman,
       l.name   AS member,
...
-- 12: Lecturers not in any committee
SELECT l.name,
       l.degree,
